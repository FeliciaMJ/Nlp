class SegmentTree:
    ...
    def __init__(self, a_list, merger_):
        """
        Description: 线段树的构造函数
        Params:
        - a_list: 用户传入的一个list（这里我们就不用以前实现的Arr类了，直接用python的list啦，如果想用的话也是一点问题都没有的～）
        - func: merge函数，用于对实现两个数合成一个数的功能（比如二元操作符加法、乘法……等等）
        """
        self._data = a_list[:]  # 所以为了不改变传入的数组，需要传其副本
    ...
    def query(self, query_left, query_right):
        ...
        Params:
        - query_left: 区间左端点的索引
        - query_right: 区间右端点的索引
        Returns:
        [quaryL, query_right]区间上的值
        """
        if query_left < 0 or query_right < 0 or query_left >= self.get_size() or query_right >= self.get_size() or query_right < query_left:  # 索引合法性检查
            raise Exception('The indexes is illegal!')
        return self._query(0, 0, self.get_size() - 1, query_left, query_right)  # 调用self._quary函数
